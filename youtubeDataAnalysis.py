...
def fig(bar,xc,yc):
    ...
    tab1, tab2 = st.tabs(["Plotly native theme(default) ", "Streamlit theme"])
    with tab1:
...
SELECT = option_menu( menu_title = None,
        options = ["About","Home","INSIGHTS & ANALYSIS"],
        icons =["bar-chart","house","toggles"],default_index=0,orientation= "horizontal",
        styles={ "container": {"padding": "0!important", "background-color": "#6739b7"},
                "icon": {"color": "white", "font-size": "20px"},
                "nav-link": {"font-size": "20px", "text-align": "center", "margin": "-1px" },
                "nav-link-selected": {"background-color": "#FF0000"}        })
if SELECT == "About":
    st.subheader("YouTube is an American online video sharing andsocial media platform headquartered in San Bruno, California, United States. Accessible worldwide, it was launched on February 14, 2005, by Steve Chen, Chad Hurley, and Jawed Karim.It is owned by Google and is the second most visited website, after Google Search.")
    col1,col2 = st.columns(2)
    with col1:
        st.image(pht)

if SELECT == "Home":
    st.subheader("YouTube has more than 2.5 billion monthly users,[citation needed] who collectively watch more than one billion hours of videos each day.As of May 2019, videos were being uploaded at a rate of more than 500 hours of content per minute.YouTube was founded by Steve Chen, Chad Hurley, and Jawed Karim.The trio were early employees of PayPal, which left them enriched after the company was bought by eBay.Hurley had studied design at the Indiana University of Pennsylvania,and Chen and Karim studied computer science together at the University of Illinois Urbana-Champaign.")
    col1,col2 = st.columns(2)
    with col2:
        st.image(pht1)
                

if SELECT == "INSIGHTS & ANALYSIS":
    ...
    query9 = "SELECT DISTINCT title,viewCount FROM video_info; "
    col9 = ['title','viewCount']
    query10 = "SELECT DISTINCT title,duration FROM video_info; "
    col10 = ['title','duration']
    ...
    if INSIGHTS:    
        ...
        if selected ==  " the total number of views for channel":
            df = df(query9,col9)
            dfs = df['viewCount'].sum()
            st.write(f'Total no of views for channel {select} is',dfs ,'views')
